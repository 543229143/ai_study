...
/** 文字置信度（"置信度：高/中/低"，兼容 markdown 加粗：**置信度**：高 / 置信度：**高**）——与百分比等价视为已给出置信度。 */
const TEXT_CONFIDENCE_RE = /(?:\*{0,2}\s*置信度\s*\*{0,2}|confidence)\s*[:：]?\s*\*{0,2}\s*(?:极高|较高|高|中|较低|低)/i;

/** 最终回答格式模板开头（"排查完成（env）— 结论：…"）：按模板结构化输出即视为已给出结论结构，不再强求置信度字样。 */
const TEMPLATE_HEADER = "排查完成（";
...
export function validateConclusion(text: string, opts: ConclusionCheckOptions = {}): ConclusionCheckResult {
  ...
  // 按格式模板开头（"排查完成（…）"）→ 视为结构化回答，不再强求置信度字样
  const structured = t.startsWith(TEMPLATE_HEADER);
  const hasConfidence = structured || CONFIDENCE_RE.test(t) || TEXT_CONFIDENCE_RE.test(t);
