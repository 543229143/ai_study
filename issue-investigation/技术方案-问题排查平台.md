...
| LLM 通道 | `opencode-go` provider | OpenAI 兼容接口 `https://opencode.ai/zen/go/v1`，模型 `deepseek-v4-flash`（默认），key 独立存 `config/pi-agent/auth.json`（不依赖 ~/.pi/agent） |
...
├── pi-agent/                    # pi 配置（auth/models，独立维护在 config/pi-agent/）
...
analysis: bun run src/index.ts                              # 监听 8700，LLM 配置读 config/pi-agent/
