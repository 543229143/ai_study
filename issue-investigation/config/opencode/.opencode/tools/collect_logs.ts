...
export default tool({
  description:
    "采集 dev/sit 环境 ES 日志（按 traceId/告警/业务键）。结果含各应用命中数与错误数，以及日志原文采样。apps[] 显式指定 = 真实采集清单（只采这些应用）；不传默认扫描全部配置应用。",
  args: {
    ...
    apps: tool.schema.array(tool.schema.string()).optional().describe("真实采集应用清单（显式传则只采这些，如 ['goa','lps']；不传默认按 scope 全量）。**一次调用即可覆盖全部需要应用，不要换 app 重复调用**"),
  },
